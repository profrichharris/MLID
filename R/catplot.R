confint.index <- function(index, level = 0.95) {
  ...
  if(class(index) != "index") stop("Object is not of class index")
  mlm <- attr(index, "mlm")
  if(is.null(mlm)) stop("Object is not multilevel")
  ...
  cint <- lapply(lvls, function(x, v = vv, sigma = sigm, z = zz) {
...
plot.confint <- function(confint) {
  ...
  if(grd[1] > grd[2]) grd[c(1,2)] <- grd[c(2,1)]
  ...
  lapply(confint, function(y) {
    ...
    if(n > 75) {
      ymid <- y[11:(n-10), ]
      while(nrow(ymid) > 55) ymid <- thin(ymid)
    ...
    plot(1:n, y$mn, ylim = c(min(y[,1:3]), max(y[,1:3])), xlab =  "Rank",
         ylab = "Scaled residual", las = 1, pch = 20, xaxt = "n", cex=0.8)
    subset <- as.integer(seq(1, n, length.out = 5))
    axis(1, at = y$i[subset], labels = y$rank[subset], cex.axis=0.9)
    abline(h = 0, lty = "dotted")
    apply(y, 1, function(x) arrows(x0 = x[5], y0 = x[2], x1 = x[5], y1 = x[3],
                                   length = 0.01, angle = 90, code = 3))
    xx <- 2:(n-1)
    subset <- unlist(lapply(xx, function(x, yy = y)
      ifelse(yy$lwr[x] > yy$upr[x + 1] & yy$upr[x] < yy$lwr[x-1], T, F)))
    subset <- c(y$lwr[1] > y$upr[2], subset, y$upr[n] < y$lwr[n-1])
    if(length(subset[subset]) > 0)
      text(x = (1:n)[subset], y = y$mn[subset], rownames(y)[subset], cex=0.6,
